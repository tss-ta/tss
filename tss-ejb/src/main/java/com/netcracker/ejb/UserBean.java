...
public class UserBean implements SessionBean {
	...
	private List<Role> toRoleList(List<Roles> roles, RoleDAO roleDAO) {
		...
		while (rolesIterator.hasNext()) {
			...
			// if (role == null) {
			// throw new IllegalArgumentException("Role with name " + rolename +
			// " doesn't exist");
			// }
			roleList.add(role);
		}
	...
	public List<UserDTO> getUsersByRolename(Roles role, int pageNumber,
			int paginationStep) {
		UserDAO dao = null;
		try {
			dao = new UserDAO();
			return toUserDTOList(dao.getByRolename(role.toString(), pageNumber,
					paginationStep));
		} finally {
			if (dao != null) {
				dao.close();
	 ...
	 */
	public List<UserDTO> searchUsersByEmail(String emailPart, int pageNumber,
			int paginationStep) {
		UserDAO dao = null;
		try {
			dao = new UserDAO();
			return toUserDTOList(dao.searchByEmail(emailPart, pageNumber,
					paginationStep));
		} finally {
			if (dao != null) {
				dao.close();
	...
	public List<UserDTO> searchCustomersByEmail(String emailPart,
			int pageNumber, int paginationStep) {
		String rolename = Roles.CUSTOMER.toString();
		return searchUsersByEmailAndRolename(emailPart, rolename, pageNumber,
				paginationStep);
	}

	public List<UserDTO> searchUsersByEmailAndRolename(String emailPart,
			String rolename, int pageNumber, int paginationStep) {
		UserDAO dao = null;
		try {
			dao = new UserDAO();
			return toUserDTOList(dao.searchByEmailAndRolename(emailPart,
					rolename, pageNumber, paginationStep));
		} finally {
			if (dao != null) {
				dao.close();
			}
	...
	private List<UserDTO> toUserDTOList(List<User> users) {
		...
		while (customersIterator.hasNext()) {
			...
			customers.add(new UserDTO(customer.getId(), customer.getUsername(),
					customer.getEmail(), toGroupNameList(customer.getGroups()),
					toEnumRolesList(customer.getRoles())));
		}
		return customers;
	}
